(function(){
  ...
  angular
    .module('calcKeyboard')
    .service('calculatorSrv', calculator);
  ...
  function calculator() {
    ...
    var service = {
      ...
      getResult: getResult,
      getCalculationString: getCalculationString,
      clear: clear
    };
    ...
          }
          var lastOperation = levels.peek().peek();
          if(lastOperation) lastOperation.addInput(input);
          collapse(0);
          reset();
          return this;
        },
    ...
    function add(number){
      InputStack.push(number, new Operation(operationData.add));
    }

    function subtract(number){
      InputStack.push(number, new Operation(operationData.subtract));
    }

    function divide(number){
      InputStack.push(number, new Operation(operationData.divide));
    }

    function multiply(number){
      InputStack.push(number, new Operation(operationData.multiply));
    }
    ...
    function closeContext(number){
      InputStack.closeContext(number);
    }

    function evaluate(number){
      InputStack.evaluate(number);
    }

    function getResult(){
      return InputStack.getPartialResult();
    }
    ...
    function getCalculationString(){
      return InputStack.getCalculationString();
    }

    /** Evaluate a calculation string */
    function evaluateString(calculation){
      ...
      evaluate(getCurrentNumber()); //include last number

      return getResult();

      function handleDigit(char){
        if(isOperator(char))        addOperation(char);
        if(isParenthesis(char))     handleParentheses(char);
        if(isNumberCharacter(char)) addCharacter(char);
      }
      ...
      function addOperation(operator){
        switch(operator){
          case '÷': divide(getCurrentNumber());    break;
          case '–': subtract(getCurrentNumber());  break;
          case 'x': multiply(getCurrentNumber());  break;
          case '+': add(getCurrentNumber());       break;
        }
      ...
      function closeParentheses(number){
        closeContext(number);
        clearCurrentNumber();
      }
      ...
      function addCharacter(character){
        setCurrentNumber(getCurrentNumber() + character);
      }
